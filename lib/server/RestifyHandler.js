...
RestifyHandler.prototype.run = function(req, res, next) {
    ...
    if(this._authMethods) {
        ...
        var sid = auth.username + '-' + auth.password;
        if(!this._authMethods.check(sid)) {
            return next(new restify.InvalidCredentialsError('Invalid credentials were provided.'));
        } else {
            req.sid = sid;
            req.session = this._authMethods.get(sid);
        }
    }
